...
export default async function EvidencePage({ params, searchParams }: PageProps) {
  ...
  return (
    <ProjectPageShell projectId={projectId} locale={locale} title={dictionary.app.evidence} eyebrow={state.data.brandName} description={zh ? "模型观察、机器推断、外部来源与实验结果分层展示。" : "Model observations, machine inference, sources, and experiments are separated."} workflowState={state.data._count}>
      ...
      <div className="grid min-w-0 grid-cols-1 items-start gap-6 [overflow-wrap:anywhere]">
        <details className="data-panel min-w-0" open={Boolean(runId || model || from || claimType || evidenceGrade || sourceStatus)}>
          <summary className="cursor-pointer px-5 py-4 text-sm font-medium">{zh ? "筛选与导出证据" : "Filter & export evidence"}<span className="ml-3 text-xs font-normal text-muted-foreground">{zh ? "按模型、时间、结论类型筛选" : "By model, date, and claim type"}</span></summary>
          <form className="grid min-w-0 grid-cols-1 gap-4 border-t border-border p-5 [&_select]:min-w-0 sm:grid-cols-3 xl:grid-cols-6">
