  ...
  useEffect(() => {
    ...
    function draw(now: number) {
      ...
      for (const type of Object.keys(SECTOR_DIR) as UniverseType[]) {
        ...
        glow.addColorStop(0, `rgba(${hue[0]},${hue[1]},${hue[2]},${0.08 * amount})`);
      ...
      if (useLod) {
        for (const cs of clusterSlots) {
          ...
          drawClusterGlow(ctx, cs.sx, cs.sy, radius, HUE[cs.cluster.type], cs.cluster.intensity * cs.fog * 0.6, 1 - cs.t);
        }
        ctx.globalAlpha = 1;
      }
      ...
      for (const item of live) {
        ...
        ctx.globalAlpha = (0.05 + s.confidence * 0.14 + s.affinity * 0.28) * item.fog * dim * reveal * item.ct;
      ...
      for (const item of detailOrder) {
        ...
        ctx.globalAlpha = (0.08 + s.affinity * 0.42) * item.fog * dim * item.ct;
        ctx.fillStyle = s.color;
